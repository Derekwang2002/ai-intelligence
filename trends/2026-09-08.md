# Current Trend State — as of 2026-09-08T00:00Z
...
### Weakening: AI 工具链的企业自托管 / 数据驻留执行面成形
- **Status:** weakening（2026-09-05 立项为 candidate；2026-09-08 降级——立项时自设的触发条件命中：本周期无第二家开发工具厂商落地自托管 agent 执行，OpenAI PSP 白皮书仍未发布。9 月窗口未结束，暂不作废；下一周期若无实质进展则 retire）
- **Confidence:** Low
...
- **Why it matters:** 对受监管与自托管取向的团队，常驻 agent 栈（云代理、MCP 集群）此前没法采用，因为执行意味着把代码与凭据送出边界。这个窗口里，「数据驻留承诺」（ZDR/EFS）与「执行面自带基础设施」（Cursor self-hosted）开始拼成完整选项——执行面选择正在成为与模型选择并列的采购轴线。低置信度的原因：证据以厂商承诺与单一产品发布为主，缺独立组织采用数据，EFS 与 Private Safety Processing 的落地时间表也尚未兑现。
...
- **What would confirm:** 第二家开发工具厂商落地自托管 agent 执行（不只是数据承诺）；≥2 个独立组织的生产案例与采用数据；OpenAI Private Safety Processing 白皮书与 Anthropic EFS 按时间表兑现
