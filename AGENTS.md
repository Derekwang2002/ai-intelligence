...
`## Tech Radar` 是当天的处置看板，写给人看，不是名单堆砌。固定格式：

- 只列「当日有变化」的条目：新增、等级调整、重要更新。每条单独一行，格式为 `- **名称**（category / 等级）：一句人话说明变了什么、为什么是这个等级`，说明遵守 6.5 节风格；新增条目在名称前加「新」标记。
- 禁止用逗号把多个名字串进同一条；禁止写「其余沿用 X 月 X 日雷达」这类要求读者翻历史日报的话。
- 小节末尾用一行汇总当前各等级条数（如 `当前雷达：ADOPT 6 / TRIAL 59 / WATCH 75`），并注明完整当前雷达以站点洞察页为准。
...
站点雷达只展示「近 30 天内有活动」的条目：活动 = 发布或出现重要更新，以 `published_at` 与 `last_updated_at` 的较晚者为准；且每个（领域 × 等级）组只上评分最高的少量代表（ADOPT / TRIAL / WATCH ≤ 6、IGNORE ≤ 4），完整条目在事件库。因此：

- 入库超过 30 天、但处置建议仍有现实意义的条目，每日运行时应复核：确认等级仍然成立，有实质新信息时刷新 `last_updated_at` 使其留在雷达上。
