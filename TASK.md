...
8. 按事件官方发布时间或实际发生日期写入或更新 `events/YYYY-MM-DD.json`。
9. 按自然日累计更新 `daily/YYYY-MM-DD.md`，不要创建 run-specific 日报。
10. 基于历史证据更新 `trends/YYYY-MM-DD.md` 和 `trends/current.md`。证据不足时明确写“本周期没有发现证据充分的新趋势”，不要强行生成趋势。
